...
- **Backend**: Node.js + Express + MongoDB + Socket.io + Zod
- **Frontend**: React 19 + Vite + TailwindCSS + Hls.js + Axios
- **Media**: SRS (Simple Realtime Server) для HLS/RTMP
- **Payments**: ЮKassa
- **Auth**: JWT access + refresh tokens в httpOnly cookies
...
| JWT_ACCESS_EXPIRES | Время жизни access token (15m) |
| JWT_REFRESH_EXPIRES | Время жизни refresh token (7d) |
| YOOKASSA_SHOP_ID | ID магазина ЮKassa |
| YOOKASSA_SECRET_KEY | Секретный ключ ЮKassa |
| PLATFORM_COMMISSION_PERCENT | Комиссия платформы (5%) |
| SRS_API_URL | URL API SRS (http://localhost:1985) |
| SRS_ORIGIN | URL HLS (http://localhost:8080) |
| SRS_RTMP_HOST | RTMP хост (rtmp://localhost/live) |

## Архитектура

- `backend/` — Express сервер с моделями, роутами, middleware, сервисами

...
export default function WatchLesson({ user }) {
  ...
  useEffect(() => {
    ...
    api(`/api/video/token/${lessonId}`).then(data => setVideoUrl(`http://localhost:3001${data.url}`)).catch(err => setError(err.message));
  }, [id, lessonId]);
  ...
  return (
    <div className="container animate-in">
      ...
      <div className="video-container">
        {videoUrl ? (
          <video controls autoPlay className="video-player" style={{ width: "100%", borderRadius: 12, background: "#000" }}>
      ...
      <div className="flex gap-4 mt-4">
        {prevLesson && <Link to={`/course/${id}/lesson/${prevLesson.id}`} className="btn btn-secondary">← {prevLesson.title}</Link>}
        {nextLesson && <Link to={`/course/${id}/lesson/${nextLesson.id}`} className="btn btn-primary">{nextLesson.title} →</Link>}
      </div>
    </div>
  );
}

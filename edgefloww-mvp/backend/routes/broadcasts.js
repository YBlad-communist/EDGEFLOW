  ...
  try {
    ...
    const enriched = await Promise.all(
      broadcasts.map(async (b) => {
        ...
        return { ...b, viewerCount: purchaseCount };
      })
    );
    res.json(enriched);
  } catch (err) {
...
router.post("/:id/purchase", authMiddleware, async (req, res, next) => {
  try {
    ...
    if (broadcast.price > 0) {
      ...
      await user.save();
      const commission = (broadcast.price * (parseInt(process.env.PLATFORM_COMMISSION_PERCENT) || 5)) / 100;
...
router.get("/:id/chat", authMiddleware, async (req, res, next) => {
  try {
    ...
    res.json(
      msgs
        .reverse()
        .map((m) => ({ ...m, username: m.userId?.displayName || m.userId?.username || "Unknown" }))
    );
  } catch (err) {
    next(err);
  }
